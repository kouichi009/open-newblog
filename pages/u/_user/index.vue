<template>
  <div>
    <nav-bar></nav-bar>
    <v-main>
      <v-container class="container-fluid px-5 py-2 mx-auto">
        <profile-header
          v-if="user"
          :user="user"
  ...
  methods: {
    ...
    async infiniteHandler($state) {
      ...
      try {
        ...
        console.log("pages/u/_user/index.vue error: ", errorMessage);
      } finally {
        if (errorMessage) {
          $state.complete();
        }
    ...
    async firstCall() {
      let returnObj = await this.$store.dispatch(`user_module/${FETCH_POSTS}`, {
        user: this.user,
